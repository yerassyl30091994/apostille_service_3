/**
 * Скрипт альтернативной (отладочной) сборки приложения
 * При запуске включает watch - для удобства разработки,
 * но если этот скрипт использовать при сборке mvn - то сборка из-за watch зависнет
 *
 * Для того чтобы запустить:
 * gulp --gulpfile gulpfile-debug.js build
 *
 * Автор
 * Дениc Митин (FLS)
 */
...
gulp.task('imgs', function() {
    return gulp.src([
        ...
        "node_modules/egov-pep-frontend/kit/_styles/images/icons/icon-contrast-pep.png"
    ])
...
gulp.task('declarations', function() {
    /*
        Если какой - то файл приложения надо подключить вручную (в целях отладки)
        то надо прописать исключение этого файла из сборки, например
            '!app/declarations/p65-09/script.js'
    */
    return gulp.src([
...
gulp.task('build-js-clean', ['html', 'components'],function() {
    return gulp.src([
        'node_modules/egov-pep-frontend/build/vendors.js',
        'node_modules/egov-pep-frontend/build/app.js',
        'node_modules/egov-pep-frontend/build/html.js',
...
gulp.task('watch', function() {
    /*
        Исключения из правил включения задачи watch надо записывать тут.
        Например, если файл script.js подключён напрямую, то лучше поставить его в исключение,
        чтоб пр измении этого файла - не срабатывал watch
        Пример:
    */
    //        gulp.watch(['app/**/*.js', '!app/declarations/p65-09/script.js'], ....

    gulp.watch(['app/**/*.js'], ['declarations', 'build-js-clean']);
    gulp.watch([ 'app/declarations/**/*.html'], ['html', 'build-js-clean']);
    gulp.watch([ 'kit/directives/**/*.js', 'kit/services/*.js', 'kit/providers/router.js', 'kit/**/*.json', 'app/**/*.json', 'i18n/**/*.json'], ['components', 'build-js-clean']);
});


gulp.task('build', ['build-js', 'build-css', 'fonts', 'imgs', 'favicon', 'perf', 'ricons', 'ricons-contrast', 'ricons-contrast-black', 'watch'], function(){
    var d = new Date();
    ...
    return fs.readFile('./node_modules/egov-pep-frontend/kit/index.tpl', 'utf8', function (err,data) {
        ...
        /*
            Здесь можно вручную добавить файлы, которые нужны для отладки
            (предварительно исключив их из задач сборки, и )
            Пример добавления:
                jsTpl += '<script src="app/declarations/номер_услуги/script.js"></script>';
         */
